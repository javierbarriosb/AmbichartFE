...
export class LayoutInitService {
  ...
  private initHeader() {
    this.layout.setCSSClass('headerContainer', this.layout.getProp('header.width') === 'fluid' ? 'container-fluid' : 'container-xxl');
  ...
  private initPageTitle() {
    ...
    if (responsive) {
      ...
      this.layout.setHTMLAttribute('pageTitle', 'data-kt-swapper-parent', `{ default: '#kt_content_container', '${responsiveBreakpoint}': '${responsiveTarget}'}`);
    }
  }
  ...
  private initToolbar() {
    ...
    if (type === 'toolbar1') {
      ...
      const heightAndTabletMobileMode = this.layout.getProp('toolbar.layouts.toolbar1.heightAndTabletMobileMode') as string;
